class Particle {
  ...
  Particle(String n_, PApplet p, PImage img_) {
    ...
    n = n_;
    minim = new Minim(p);     
    sound = minim.loadFile("voiceSound (" + n + ").mp3");
    sound.setGain(0);
  ...
  void enter() {
    ...
    if (toggle == 0) {
      position = new PVector(0, random(height/3, height - (height/3)));
      acceleration = new PVector(stepSize,0);
    } else {
      position = new PVector(width, random(height/3, height - (height/3)));
      acceleration = new PVector(-stepSize,0);
    }
  }
